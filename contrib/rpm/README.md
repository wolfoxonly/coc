...
The RPM spec file provided here is for Crowncoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Crowncoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Crowncoin git repository](https://github.com/Crowncoin/Crowncoin) but are not part of
...
Currently only `Source10` is used, for the example `Crowncoin.conf` file.
...
in the Crowncoin git repository. At present only an SVG version of the Crowncoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Crowncoin
...
needed when 0.12.1 is released, a proper fix is already in the Crowncoin git
master branch.
...
idea to build Crowncoin Core against a static version of that library compiled
...
The `Crowncoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Crowncoin-server` package. At this point, most RPM based
...
The `Crowncoin-server` package should have SELinux support. How to properly do
...
Crowncoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Crowncoin/). He has been
packaging Crowncoin for Fedora at least since 2012.
...
4. I renamed the `Crowncoin` package that contains the Qt GUI to `Crowncoin-core` as
that appears to be how the general population refers to it, in contrast to
`Crowncoin-xt` or `Crowncoin-classic`. I wanted to make sure the general population
