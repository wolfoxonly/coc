...
###   Crowncoincore.org and Crowncoin.org and compares them.
...
WORKINGDIR="/tmp/Crowncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://Crowncoincore.org"
HOST2="https://Crowncoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="Crowncoin-core-"
RCVERSIONSTRING="rc"
...
if [ $? -ne 0 ]; then
   echo "Crowncoin.org failed to provide signature file, but Crowncoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "Crowncoin.org and Crowncoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Crowncoin Core binary release signing key installed?"
   fi
