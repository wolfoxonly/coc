...
// Copyright (c) 2011-2015 The Crowncoin developers
...
void ThreadScriptCheck() {
    RenameThread("Crowncoin-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in Crowncoin
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Crowncoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 21/2/2018 COC genesis";// "Matonis 07-AUG-2012 Parallel Currencies And The Roadmap To Monetary Freedom";
        CTransaction txNew;
        txNew.nTime = 1519213742;//
        txNew.vin.resize(1);
        txNew.vout.resize(1);
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          printf("POW pindexPrev.nHeight===%d\n", pindexPrev->nHeight);
           ...
           {
		int year =  std::ceil(pindexBest->nHeight / (24 * 60 / 2 * 365));//zxb产量衰减部分
//  float rate = 15*pow(0.66,year);//zxb
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("CrowncoinMiner : proof-of-work not meeting target");
...
#ifdef TESTING
void CrowncoinMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void CrowncoinMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    {
        if(!fProofOfStake) printf("CrowncoinMiner terminated\n");
        throw;
    }
}

void GenerateCrowncoins(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&CrowncoinMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&CrowncoinMiner, pwallet, false));
#endif
}
