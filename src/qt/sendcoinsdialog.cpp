    ...
    model(0)
{
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::CrowncoinAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(CrowncoinUnits::formatWithUnit(CrowncoinUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
#else
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(CrowncoinUnits::formatWithUnit(CrowncoinUnits::BTC, rcp.amount), rcp.label.toHtmlEscaped(), rcp.address));
#endif
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(CrowncoinUnits::formatWithUnit(CrowncoinUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    // URI has to be valid
    if (GUIUtil::parseCrowncoinURI(uri, &rv))
    {
        CCrowncoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(CrowncoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CCrowncoinAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
