...
{
    widget->setMaxLength(CrowncoinAddressValidator::MaxAddressLength);
    widget->setValidator(new CrowncoinAddressValidator(parent));
...
bool parseCrowncoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || (uri.scheme() != QString("Crowncoin") && uri.scheme() != QString("ppcoin")))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CrowncoinUnits::parse(CrowncoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
